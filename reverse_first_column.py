"""
Usage:
$ python reverse_first_column.py folder_to_reverse
Will create a new folder named "reversed_N", where N is a timestamp
You can edit this code to reverse different columns.
"""
...
try:
    ...
    for i in contents:
        ...
        if file_extension == "lpa":
            #first read the lpa file and put the reversed value of the first column
            #in the temp.txt file
            with open(os.path.join(input_dir,i), 'r') as f:
                #for line in filerev(f):
                 for line_a, line_b in itertools.izip(filerev(f), open(os.path.join(input_dir,i), 'r')):
                    rev_kp = "%s" % (line_a.split(";")[0])
                    pieces = line_b.split(";")
                    final_line = "%s;%s;%s;%s" % (rev_kp, pieces[1],pieces[2],pieces[3])
